...
- **Components**: Each needs `name`, `type`, type-specific fields. Check `type` is one of: helm_chart, terraform_module, docker_build, container_image, kubernetes_manifest, job, pulumi. Also check for `take_ownership`, `build_timeout`, `deploy_timeout`. For pulumi components, `runtime` is required. For kubernetes_manifest components, check kustomize fields: `path`, `patches`, `enable_helm`, `load_restrictor`
